...
public class ProfileValidator {
    ...
    public UserProfile validate(MultiValueMap<String, String> formData) {
        ...
        Boolean isExits =  profileRepo.findByEmail(email).isPresent();

        UserProfile userProfile = null;

        if(isExits){

            userProfile =  profileRepo.findByEmail(email).orElseThrow(()->new RuntimeException("Exception while fetching profile"));
            ...
            AppUser appUser = appUserRepository.findByEmail(email).orElseThrow(()->new RuntimeException("Exception while fetching user account details"));

            String dob = formData.getFirst("dob");
            Gender gender = Gender.valueOf(formData.getFirst("gender"));
            userProfile = new UserProfile();

            userProfile.setFirstName(appUser.getFirstName());
            userProfile.setLastName(appUser.getLastName());
            userProfile.setEmail(appUser.getEmail());
            userProfile.setId(appUser.getId());
